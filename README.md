...
脚本可自动检测常见本地代理端口，测试 OpenAI 接口连通性，写入当前用户代理变量，并重启 Microsoft Store 版 Codex。
...
双击 `outputs/一键修复Codex重连.bat` 执行修复并重启 Codex。
...
- `outputs/一键修复Codex重连.bat`：双击入口及状态、回滚命令。
- `outputs/修复Codex重连.ps1`：代理检测、测试、环境变量备份与恢复、Codex 重启逻辑。
- `work/`：临时测试产物目录。
